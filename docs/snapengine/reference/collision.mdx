...
`collider.getWorldBoundsSnapshot()` returns a `ColliderBounds`: a
[`Bounds`](/docs/snapengine/reference/geometry) plus `center` and `radius`.
For every shape, `x`/`y` are the top-left corner and `width`/`height` span
the bounds, so a circle's bounds are its enclosing square. `center` is the
rectangle center or the circle/point position.

```ts
import { rectsIntersect } from "@snap-engine/core/geometry";

const a = colliderA.getWorldBoundsSnapshot();
const b = colliderB.getWorldBoundsSnapshot();
rectsIntersect(a, b);
```
...
| `collider.getWorldBoundsSnapshot()` | Returns the cached world `ColliderBounds`. |
