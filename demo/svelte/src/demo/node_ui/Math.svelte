...
<Node bind:this={node} className="flow-math" LineSvelteComponent={DemoLine}>
    <div class="flow-math-container" id="op-container">
        <div>
            <button class="op" class:active={operation == "+"} onclick={() => {setOperation("+")}}><h1>+</h1></button>
            <button class="op" class:active={operation == "-"} onclick={() => {setOperation("-")}}><h1>-</h1></button>
            <button class="op" class:active={operation == "*"} onclick={() => {setOperation("*")}}><h1>*</h1></button>
            <button class="op" class:active={operation == "/"} onclick={() => {setOperation("/")}}><h1>/</h1></button> 
        </div>
    ...
    <div class="flow-math-container">
         {#each Object.values(inputValues) as input (input.uuid)}
            <div class="flow-math-input">
                <Connector name={`input-${input.uuid}`} maxConnectors={1} allowDragOut={false} /> 
                <div class="flow-math-input-value">
                    <input type="number" oninput={(e) => calculate(input.uuid, (e.target as any).value)} />
                    <button class="input-add-remove" onclick={() => {removeInput(input.uuid);}}>-</button>
                </div>
            </div>
         {/each}
         <button class="input-add-remove add-input" style="right: 0;" onclick={addInput}>+</button>
    </div>
</Node>
